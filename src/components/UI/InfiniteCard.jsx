...
const InfiniteCard = ({data, isActive, open, close, type}) => {
  if (!data.poster_path) {
    return
  }
  ...
  return (
    <article>
      <article
        ...
        className="flex flex-col gap-3 mb-2 overflow-hidden cursor-pointer "
      >
        <img
      ...
      {isActive && <MovieModel type={type} id={data.id} close={close} />}
    </article>
  )
}
